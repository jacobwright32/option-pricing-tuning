...
class PricingModel:
    ...
    def generate_signal(self, chain, price_history):
        ...
        range_boost = max(0.0, min(0.20, (range_5d - 0.03) * 7.0))
        ...
        if atm_puts.sum() > 0 and atm_calls.sum() > 0:
            ...
            pc_boost = max(0.0, pc_spread * 70.0)
        else:
            pc_boost = 0.0
        ...
        if iv_rv_ratio > 1.85 and ret_5d < -0.015 and dist_from_low < 0.03:
            ...
            return (0.50 + skew_boost + accel1 + pc_boost + range_boost) * low_scale * coherence
        elif iv_rv_ratio > 1.6 and ret_5d < -0.040 and dist_from_low < 0.03:
            ...
            accel = min(0.25, max(0.0, ret_5d / (ret_10d + 1e-8) - 0.3) * 0.9) if ret_10d < -0.01 else 0.0
            return (0.03 + skew_boost + term_boost + accel) * low_scale * coherence
        else:
            return 0.0
