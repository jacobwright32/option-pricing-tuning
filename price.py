...
class PricingModel:
    ...
    def generate_signal(self, chain, price_history):
        ...
        if atm_puts.sum() > 0 and atm_calls.sum() > 0:
            ...
            pc_boost = max(0.0, pc_spread * 12.0)
        else:
            pc_boost = 0.0
        ...
        if iv_rv_ratio > 1.85 and ret_5d < -0.015 and dist_from_low < 0.03:
            ...
            return (0.55 + skew_boost * 0.3 + term_boost * 0.3 + accel2 + pc_boost * 0.3) * low_scale * coherence
            ...
            return (0.4 + skew_boost + term_boost + accel + pc_boost * 0.5) * low_scale * coherence
        else:
            return 0.0
